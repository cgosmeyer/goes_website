...
sub find_data {
    ...
    for ($year=$start_year; $year<=$end_year; $year=$year+1) {
        for ($doy=$start_doy; $doy<=$end_doy; $doy=$doy+1) {
            if (length($doy) == 1) {
                $doy = '00' . $doy;
            }
            if (length($doy) == 2) {
            ...
            $search_str = $path_to_data . $mission . '/' . $inr_temp . '/' . uc($mission) . '_' . uc($inr_temp) . '_metrics_' . $year . '_' . $doy . '*csv';
...
sub ingest_data {
    ...
    foreach $search_file (@search_files) {
        if ($search_file ne '') {
            ...
            if ($results[0] == '') {
                print "File not found in database! Ingesting...";
